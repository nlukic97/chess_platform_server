...
function appMaker() {
    ...
    const Rooms = new ChessRoom_1.RoomsInit();
    ...
    io.on("connection", (socket) => {
        ...
        if (roomIndex > -1) {
            ...
            Rooms.rooms[roomIndex].players.push((0, Player_1.Player)(socket.id));
            ...
            if (Rooms.rooms[roomIndex].players.length === 2) {
                ...
                Rooms.rooms[roomIndex].chess = new chess_js_1.Chess(Rooms.rooms[roomIndex].fen);
                ...
                const assignedData = (0, GetPieces_1.GetPieces)(Rooms.rooms[roomIndex].chess.turn());
                for (let i = 0; i <= 1; i++) {
            ...
            let newFen = (0, fullFenValidation_1.fullFenValidation)(socket.handshake.query.fen)
                ...
                : undefined;
            Rooms.rooms.push((0, ChessRoom_1.ChessRoom)(socketRoomId, newFen, (0, Player_1.Player)(socket.id)));
        }
        ...
        socket.on("make-move", (data) => {
            ...
            let roomIndex = Rooms.rooms.findIndex((room) => room.id === submittedRoomId);
            ...
            let player = Rooms.rooms[roomIndex].players.find((player) => player.socketId === socket.id);
            if (player.playersTurn === true) {
                if ((0, validateMove_1.validateMove)(Rooms.rooms[roomIndex].chess, data) === true) {
                    ...
                    if (Rooms.rooms[roomIndex].chess.game_over()) {
                        ...
                        if (Rooms.rooms[roomIndex].chess.in_checkmate()) {
                            ...
                            // stalemate
                            io.in(Rooms.rooms[roomIndex].id).emit("game-over", (0, GameOutcome_1.GameOutcome)("stalemate"));
                        }
                        else if (Rooms.rooms[roomIndex].chess.in_threefold_repetition()) {
                            // threefold_repetition
                            io.in(Rooms.rooms[roomIndex].id).emit("game-over", (0, GameOutcome_1.GameOutcome)("threefold-repetition"));
                        }
                        else if (Rooms.rooms[roomIndex].chess.in_draw()) {
                            io.in(Rooms.rooms[roomIndex].id).emit("game-over", (0, GameOutcome_1.GameOutcome)("draw"));
                        }
                        else {
                            // other reason for game to be over
                            io.in(Rooms.rooms[roomIndex].id).emit("game-over", (0, GameOutcome_1.GameOutcome)("other"));
                        }
                        for (let i = 0; i <= 1; i++) {
